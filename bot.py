try:
    ...
    def ras(text):
        ...
        for it in cur.fetchall():
            try:
                ...
                fail +=1
        sms(5073415776, "Рассылку получило - " + str(succes) + " пользователей")
        sms(5073415776, "Заблокировали бота - " + str(fail) + " пользователей")
    ...
    def get_text_messages(message):
        ...
        if ban == 0:
            ...
            bot.send_message(sender, f'Ваш ID: `{sender}` \nДля оплаты картой пишите:\n@nakrut_ca', parse_mode='Markdown')
        elif mess[0:3] == 'бан':
            try:
        ...
        if call.message:
            ...
            for oo in mm:
                if call.data == f'{oo[2]}:{oo[1]}':
                    kl5 = 1
                    if str(oo[1]) != str(call.message.chat.id):
                        ...
                        if bal2 >= int(oo[0]):
                            ...
                            if fgfg == 1:
                                ...
                                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text= f"Игрок: {call.message.from_user.first_name} \nВыигрыш: " + str(int(oo[0] * 2)) + " руб. 💰")
                                sms(oo[1], f'Игрок: {call.message.from_user.first_name} \nВаша ставка проиграна ☹')
                            else:
                        ...
                        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text= "Нельзя играть с самим собой ⛔")
