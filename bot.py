...
bot = telebot.TeleBot('5248374689:AAGnYZrBestJuzc0flVaZ98iqyXFnPT2Bro')
print('Работает!')
...
def get_text_messages(message):
    ...
    if ban == 0:
        ...
        for yyy in j:
            if ooo < 5:
                ooo+=1
                igra = types.InlineKeyboardMarkup(row_width=2)
                igt = types.InlineKeyboardButton('Играть.', callback_data=f'{yyy[2]}:{yyy[1]}')
                igra.add(igt)
                sms1(sender, f'Cумма: {yyy[0]} руб.', igra)
        if ooo == 0:
        ...
        if ob_sum >= 0 and ob_sum < 10:
            ...
            yrov = 'Сильный.'
        bot.send_message(sender, f'👤 ID: `{sender}` \n♦ Уровень: {yrov}\n💰 Общая сумма пополнений: {ob_sum} руб.', parse_mode='Markdown')
            ...
            if int(mess) >= 5:
                if int(mess) <= 50:
                    ...
                    if bal2 >= int(mess):
                        ...
                        cur.execute(f"""INSERT INTO spis (cym, id, id_stav) VALUES ({int(mess)}, {sender}, {id_st});""")
                        con.commit()
