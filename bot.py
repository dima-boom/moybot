try:
    import vk_api, requests, time, threading, psycopg2, os
    ...
    con = psycopg2.connect(
      database="d8pltnnl96e4hi", 
      user="xvvqyunsmcyqaw", 
      password="7cf619a601c3da1b6070ada8c063d859c7d54727f90198a4ba7769e27e5cd9eb", 
    ...
    def new_polz(send):
        # Добавление записи
        cur.execute(f"SELECT bal FROM tab WHERE id = '{str(send)}'")
        if str(cur.fetchall()) == '[]':
            cur.execute(f"""INSERT INTO tab (id, bal, clava, rozg) VALUES ({int(send)}, 3, 0, 0);""")
            con.commit()
        else:
            pass
    def ras(text):
